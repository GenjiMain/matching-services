        ...
        */
        ...
        if (isLeaf(name1) & isLeaf(name2)) {
            if (name1.equals(name2)) {
                ...
                score = weights.stream().map(w->Math.pow(w,2)).reduce((double) 0,Double::sum);

            } else {
                /*
                score += 0.0;
                List <String >antecedent_name1 =   getAllAntecedent(name1) ;
                List <String >antecedent_name2 =  getAllAntecedent(name2) ;
                */
                score += 0.0;
                while (!gotToRoot) {
